...
export function createMcpHandler(factory: McpServerFactory, options: CreateMcpHandlerOptions = {}): McpHttpHandler {
    ...
    const closeTracked = new WeakSet<Server>();
    let closed = false;
    ...
    async function serveModern(route: InboundModernRoute, request: Request, authInfo: AuthInfo | undefined): Promise<Response> {
        ...
        // The factory contract asks callers for a fresh instance per request, but
        // avoid stacking an unbounded onclose wrapper chain if a caller reuses one.
        inflight.add(server);
        if (!closeTracked.has(server)) {
            const previousOnClose = server.onclose;
            closeTracked.add(server);
            server.onclose = () => {
